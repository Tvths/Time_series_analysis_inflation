...
make_tsibble <- function(plot_df, n_months = 180) {
  plot_df %>%
    ...
    select(GEO, ym, HICP_INDEX) %>%
    as_tsibble(key = GEO, index = ym) %>%
    group_by_key() %>%
    mutate(
      infl_mom = 100 * (HICP_INDEX / dplyr::lag(HICP_INDEX) - 1),
      infl_yoy = 100 * (HICP_INDEX / dplyr::lag(HICP_INDEX, 12) - 1)
    ) %>%
    ungroup()
}
...
############################################################
data_path <- "C:/Users/timce/OneDrive/Dokument/Rcode_2024_2025/732G52_TimeSeries/Projekt-2025/estat_prc_hicp_midx_filtered_en.csv"
...
p_se <- infl_mom_df %>%
  ...
  labs(
    ...
    subtitle = "Månatlig inflation – Sverige",
    x = "Tid", y = "Procent"
  ) +
...
p_de <- infl_mom_df %>%
  ...
  labs(
    title = "Inflationstakt (transformerad från indexnivå)",
    subtitle = "Månatlig inflation – Tyskland",
    x = "Tid", y = "Procent"
  ) +
  coord_cartesian(ylim = y_rng) +   # ta bort raden om du vill ha “auto-zoom”
  theme_bw()
...
hicp %>%
  filter(GEO == "SE", !is.na(infl_mom)) %>%
  model(stl = STL(infl_mom)) %>%
  components() %>%
  autoplot() +
  labs(
    title = "STL-dekomposition: Månatlig inflation – Sverige",
    x = "Tid",
    y = "Procent"
  )

hicp %>%
  filter(GEO == "DE", !is.na(infl_mom)) %>%
  model(stl = STL(infl_mom)) %>%
  components() %>%
  autoplot() +
  labs(
    title = "STL-dekomposition: Månatlig inflation – Tyskland",
    x = "Tid",
    y = "Procent"
  )
...
plot_fitted_vs_actual <- function(fit_obj, country_code) {
  ...
  ggplot(aug_long, aes(x = ym, y = value, colour = type, linetype = type)) +
    geom_line() +
    ...
    labs(title = paste0("Train+validation: fitted vs actual (", country_code, ")"),
         x = "Tid", y = "Månatlig inflation (%)",   # <-- ändrad
         colour = "", linetype = "")
}
